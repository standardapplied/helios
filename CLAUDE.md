...
| **Hooks + HookOutcome** | Seven phase-specific non-sealed sub-interfaces of `Hook` (`OnUserMessageHook`, `PreModelTurnHook`, `PostModelTurnHook`, `PreToolUseHook`, `PostToolUseHook`, `PreStopHook`, `OnStreamEventHook`) returning a `HookOutcome` (`Continue` / `MutateInput` / `Block` / `Inject` / `Stop`). Not every outcome is meaningful at every phase; the phase Javadoc documents which fire. `HookRegistry` catches `RuntimeException` from hooks (logs WARNING, treats as Continue), but `Error` subtypes escape — see the Loop crash semantics row |
...
| **Structured Output Resilience** | Provider `parseStructuredContent` always reads to `Map` first, then runs `core.schema.SchemaValidator` against the configured schema. Shape mismatches surface as `StructuredOutputParseException` carrying a per-field diff (e.g. `provenance[3].sources[0].title is required but missing`) rather than an opaque "Failed to parse structured output". The session loop pattern-matches on this exception when an `OutputSchema` run gets a non-conforming response, injects a corrective USER turn, and re-iterates within `maxIterations` — same self-correction shape RLM `submit()` used to have. `SchemaValidator` is public in `core/schema` and reused by typed `runBlocking` parsing |
...
**Subprocess module restriction (`SubprocessModules`, L3).** Sealed interface (`Unrestricted` / `Restricted` records) controlling the subprocess JVM's `--limit-modules` flag. Three factories: `unrestricted()` (default — all JDK modules observable), `minimal()` (required roots only — `java.base`, `java.compiler`, `jdk.compiler`, `jdk.jshell`, `ai.singlr.repl`; strips `java.sql`, `java.naming`, `java.scripting`, `java.desktop`, `jdk.httpserver`, etc.), `allowingExtras(String...)` (required + named extras). Wired through `JvmSandboxConfig.withSubprocessModules(...)`. **Bootstrap-transitive-closure limit**: cannot strip modules the bootstrap itself transitively requires — today `ai.singlr.core` requires `java.net.http` via `HttpClientFactory`, so `java.net.http` stays observable under any L3 setting. Deployers who need to deny those modules reach for L2 `deniedPackages` instead. Composes with L2 enforcement — limit-modules eliminates whole categories at compile time, the verifier catches the rest at load time. `JvmSandbox.shouldPropagateJvmArg` now also filters `--add-modules` and `--limit-modules` from inherited parent args so Maven Surefire's `--add-modules=ALL-MODULE-PATH` doesn't defeat the restriction.
